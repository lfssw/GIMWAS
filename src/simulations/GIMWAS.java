...
public class GIMWAS {
	...
	public static HashMap<String, Double> RSID_Weights_Dic;
	public static HashMap<String, String> RSID_Ref_Dic;
	public static HashMap<String, String> RSID_Alt_Dic;
	public static HashMap<String, String> RSID_Chr_Dic;
	public static HashMap<String, String> RSID_Pos_Dic;
	static double pheno_unit = 1.0;
	private static final String GIMWAS_USAGE = "Usage:\n"
			+ "java -jar GIMWAS.jar GIMWAS\n"
			+ "-format csv|plink\n"
			+ "-input_genotype INPUT_GENOTYPE_FILE\n"
			+ "-input_phenotype INPUT_PHENOTYPE_FILE\n"
			+ "-input_phenotype_column INPUT_PHENOTYPE_COLUMN_START:2|6\n"
			+ "-input_phenotype_type PHENOTYPE_TYPE: continuous|binary\n"
			+ "-summary_info SUMMARY_INFORMATION_FILE\n"
			+ "-microbe INPUT_MICROBE_FEATURE_ID\n"
			+ "-plink PLINK_BINARY_FILE_PATH\n"
			+ "-Rscript RSCRIPT_BINARY_FILE_PATH\n"
			+ "-output_folder OUTPUT_FOLDER_PATH\n";

	private static void printUsage(String intro) {
...
//       for (int k = 0; k < sample_size; k++) sample_ids[k] = temp[k + 2];
		...
		while ((line = br_plink2.readLine()) != null) {
			line = line.replace("\n", "").replace("\r", "").replace(" ", "");
			String[] tmp_arr = line.split("\t");
			Integer.parseInt(tmp_arr[3]);
			if (RSID_Weights_Dic.containsKey(tmp_arr[1])) {
				String rs = tmp_arr[1];
				String ref = RSID_Ref_Dic.get(tmp_arr[1]);
				String alt = RSID_Alt_Dic.get(tmp_arr[1]);
				bw_setid.write("SET\t" + rs + "\n");
				...
				ss = rs + "\t";
				if (RSID_Weights_Dic.containsKey(tmp_arr[1])) {
					ss = ss + Double.toString(RSID_Weights_Dic.get(tmp_arr[1]));
				} else {
					ss = ss + "0.0";
				}
				bw_en_weight.write(ss + "\n");
	...
	public static void get_en_info(String info_file) throws IOException, InterruptedException {
		String line = null;
		boolean OK = false;
		RSID_Weights_Dic = new HashMap<String, Double>();
		RSID_Ref_Dic = new HashMap<String, String>();
		RSID_Alt_Dic = new HashMap<String, String>();
		RSID_Chr_Dic = new HashMap<String, String>();
		RSID_Pos_Dic = new HashMap<String, String>();
		...
		while ((line = br_info.readLine()) != null) {
			...
			if (tmp_arr[0].equals(Gene_ENSG_ID)) {
				OK = true;
				EN_SNPs = new int[tmp_arr.length - 1];
				EN_SNP_Weights = new double[tmp_arr.length - 1];
				for (int i = 1; i < tmp_arr.length; i++) {
					String[] tmp_arr2 = tmp_arr[i].split("_");
					RSID_Weights_Dic.put(tmp_arr2[0], Double.parseDouble(tmp_arr2[5]));
					RSID_Ref_Dic.put(tmp_arr2[0], tmp_arr2[3]);
					RSID_Alt_Dic.put(tmp_arr2[0], tmp_arr2[4]);
					RSID_Chr_Dic.put(tmp_arr2[0], tmp_arr2[1]);
					RSID_Pos_Dic.put(tmp_arr2[0], tmp_arr2[2]);
				}
			}
		}
		br_info.close();
	...
	private static boolean looksLikeChromosome(String token) {
		if (token == null || token.isEmpty()) {
			return false;
		}
		String normalized = token.toLowerCase();
		return normalized.startsWith("chr") || normalized.equals("x") || normalized.equals("y")
				|| normalized.equals("m") || normalized.equals("mt") || isIntegerToken(token);
	}

	private static boolean looksLikeInterval(String token) {
		return token != null && token.contains(":") && token.contains("-");
	}

	private static String[] parseInterval(String token) throws IOException {
		String[] chr_and_region = token.split(":");
		if (chr_and_region.length != 2) {
			throw new IOException("Unsupported interval format: " + token);
		}
		String[] bounds = chr_and_region[1].split("-");
		if (bounds.length != 2 || !isIntegerToken(bounds[0]) || !isIntegerToken(bounds[1])) {
			throw new IOException("Unsupported interval bounds: " + token);
		}
		return new String[] { chr_and_region[0], bounds[0], bounds[1] };
	}

	private static WeightSnpInfo parseWeightSnpInfo(String token, String fallbackChromosome) throws IOException {
		String[] parts = token.split("_");
		if (parts.length < 5) {
			throw new IOException("Unsupported SNP weight format: " + token);
		}

		WeightSnpInfo snp_info = new WeightSnpInfo();
		snp_info.weight = Double.parseDouble(parts[parts.length - 1]);
		snp_info.alt = parts[parts.length - 2];
		snp_info.ref = parts[parts.length - 3];
		if (!isIntegerToken(parts[parts.length - 4])) {
			throw new IOException("Unsupported SNP position in weight field: " + token);
		}
		snp_info.position = Integer.parseInt(parts[parts.length - 4]);

		int rsid_end = parts.length - 4;
		snp_info.chromosome = fallbackChromosome;
		if (parts.length >= 6 && looksLikeChromosome(parts[parts.length - 5])) {
			snp_info.chromosome = parts[parts.length - 5];
			rsid_end = parts.length - 5;
		}

		snp_info.rsid = String.join("_", Arrays.copyOfRange(parts, 0, rsid_end));
		if (snp_info.rsid.isEmpty()) {
			throw new IOException("Unsupported SNP id in weight field: " + token);
		}
		return snp_info;
	}

	private static class WeightSnpInfo {
		String rsid;
		String chromosome;
		int position;
		String ref;
	...
	public static void get_result(String out_folder) throws IOException, InterruptedException {
		...
		BufferedReader br_info = new BufferedReader(new FileReader(out_folder + "GIMWAS.result"));
		while ((line = br_info.readLine()) != null) {
			line = line.replace("\n", "").replace("\r", "");
			String[] tmp_arr = line.split("\t");
			...
			} else if (tmp_arr[0].equals("P_value")) {
		...
		//		System.out.println("-------------------------------------");
		System.out
				.println("The p-value of GIMWAS on variable " + Gene_ENSG_ID + ":\t" + Double.toString(p_value_enweights));
		System.out.println("=====================================");
...
//        bw.write(ss+"\n");
			...
			bw.write(ss + "\n");
			ss = "ss= paste(\"P_value\",toString(res_en_weight$results$P.value),  sep = \"\\t\")";
			bw.write(ss + "\n");
		ss = "write(ss ,\"" + out_folder + "GIMWAS.result\")";
		bw.write(ss + "\n");
		bw.close();
	}
...
//					ss_eqtl= rs_dict.get(pos)+"\t1.0";
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		System.out.println("Welcome to GIMWAS! We're excited to have you here. If you need any help or have any questions, "
				+ "feel free to ask Fei Li (lifeiwsy@outlook.com) or Chen Cao (caochen@njmu.edu.cn).\n");
		
		if (args.length == 0) {
			printUsage("GIMWAS: Genetically Imputed Microbiome-Wide Association Study.");
			System.exit(0);
		}
		String function = args[0];

		if (args[0].equals("GIMWAS")) {
			final DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
			if (args.length < 14) {
				printUsage("Genetically Imputed Microbiome-Wide Association Study.");
				System.exit(0);
			} else {
				...
				for (int k = 1; k < args.length; k++) {
					if (args[k].startsWith("-")) {
						if (args[k].equals("-format"))
							...
							column = args[k + 1];
						else if (args[k].equals("-summary_info"))
							info_file = args[k + 1];
						else if (args[k].equals("-microbe"))
							gene_id = args[k + 1];
				...
				if (format == null) {
					...
					System.out.println("Please provide the absolute path for the summary information file!");
					System.exit(0);
					...
					System.out.println("Please input microbe id!");
					System.exit(0);
				} else if (plink == null) {
				...
				if (format.equals("csv")) {
					...
					select_plink(genotype_file, phenotype_file, output_folder, Integer.parseInt(column));
						System.out.println("Identifying Variants ... ...\t"
								+ dtf.format(LocalDateTime.now()));
				} else {
